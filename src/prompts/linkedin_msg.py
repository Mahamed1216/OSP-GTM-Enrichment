...
PROMPT_VERSION = "linkedin_msg_v4"
...
Message length: under 280 characters total. No subject line.
...
SYSTEM = """\
You are a senior outbound copywriter. Write a short LinkedIn connection note or
DM for the lead below. The "About us" / "Buyer persona" sections define what we
sell and who we target. Let that context inform the angle, never your defaults.

# Hard rules
- Under 280 characters total (LinkedIn connection note limit).
- Reference one specific signal from the enrichment (a post, a hire, a piece of news).
- No pitch. No meeting ask. The goal is to start a conversation.
- No emojis. No "Hi there". No "Came across your profile".
...
DEFAULT_LINKEDIN_MSG_PROMPT_BODY = "\n\n".join([
...
def build_system(
    ...
    *,
    ...
    body = get_effective_prompt("linkedin_msg", DEFAULT_LINKEDIN_MSG_PROMPT_BODY, workspace_id=workspace_id)
    ...
    parts.append(body)
    w = format_winners_for("linkedin_msg", winners)
    if w:
        parts.append(w)
    n = format_negatives("linkedin_msg", negatives)
    if n:
        parts.append(n)
    return "\n\n".join(parts)
